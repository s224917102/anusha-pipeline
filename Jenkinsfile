...
echo "[BUILD] Re-tag local images → Docker Hub targets"
...
# --- Start DB services (mirrors your GH Actions services) ---
...
echo "[TEST] Wait for DB health"
wait_healthy () {
  local name="$1"
  for i in $(seq 1 30); do
    status=$(docker inspect --format '{{json .State.Health.Status}}' "$name" 2>/dev/null || echo '"starting"')
  ...
  echo "ERROR: $name not healthy in time"; docker logs "$name" || true; return 1
}
wait_healthy product_db
wait_healthy order_db

# --- Python venv like GH runner ---
python3 -m venv .venv_ci
. .venv_ci/bin/activate
python -m pip install -U pip

# Helper to run tests for a service dir if it exists
run_tests () {
  local SRV_DIR="$1"
  local XML_OUT="$2"
  local PG_PORT="$3"
  ...
  if [ -d "$SRV_DIR/tests" ]; then
    echo "[TEST] Installing dependencies for $SRV_DIR"
    [ -f "$SRV_DIR/requirements.txt" ] && pip install -r "$SRV_DIR/requirements.txt"
    [ -f "$SRV_DIR/requirements-dev.txt" ] && pip install -r "$SRV_DIR/requirements-dev.txt" || true
    pip install 'pytest>=8,<9' httpx
    ...
    echo "[TEST] Running pytest in $SRV_DIR"
    pytest "$SRV_DIR/tests" -q --maxfail=1 --disable-warnings --junitxml="$XML_OUT"
  else
    echo "[TEST] No tests directory in $SRV_DIR — skipping"
  fi
}

# Prefer your week07 path if present, else default to backend path (both supported)
PRODUCT_DIR_PATH="${PRODUCT_DIR}"
[ -d "week07/example-2/backend/product_service" ] && PRODUCT_DIR_PATH="week07/example-2/backend/product_service"

run_tests "$PRODUCT_DIR_PATH" "product_unit.xml" 5432 "products"
run_tests "${ORDER_DIR}"      "order_unit.xml"   5433 "orders"
...
'''
      ...
      post {
        always {
          junit allowEmptyResults: true, testResults: 'product_unit.xml,order_unit.xml'
...
docker run --rm -v "$(pwd)":/src aquasec/trivy:0.55.0 fs --exit-code 1 --severity HIGH,CRITICAL /src || true

echo "[SECURITY] Trivy image scans"
for img in ${PRODUCT_IMG}:${IMAGE_TAG} ${ORDER_IMG}:${IMAGE_TAG} ${FRONTEND_IMG}:${IMAGE_TAG}; do
  docker run --rm aquasec/trivy:0.55.0 image --exit-code 1 --severity HIGH,CRITICAL "$img" || true
done
'''
      }
    }
